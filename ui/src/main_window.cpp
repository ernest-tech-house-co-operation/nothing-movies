...
MainWindow::MainWindow(TmdbBridge* tmdbBridge, SearchBridge* searchBridge, AppController* appController,
                        ...
                        HomepageBridge* homepageBridge,
                        VendorBridge* vendorBridge,   // NEW parameter
                        QWidget* parent)
    : QMainWindow(parent), homepageBridge_(homepageBridge), vendorBridge_(vendorBridge) {
    initUiResources();
    ...
    quickView_->rootContext()->setContextProperty("homepageBridge", homepageBridge);
    quickView_->rootContext()->setContextProperty("vendorBridge", vendorBridge);   // NEW
