...
// True if the Nothing Browser binary is installed (checked via
// VendorManager's install location), independent of whether it's
// currently running.
bool isAvailable();
...
public:
    ...
    // connection-key auth.
    //
    // Registers itself with vendor_updater::VendorManager on first call
    // (name "nothing-browser") so the binary shows up in the Settings ->
    // External Tools UI alongside every other vendor dependency, instead
    // of being a special case nothing else can see. Owns the entire
    // vendor install/update lifecycle from the caller's point of view: if
    // the binary isn't present yet, does a synchronous first-time fetch
    // via the registered VendorUpdater before attempting to spawn (so a
    // fresh install doesn't race the background watcher), then relies on
    // VendorManager's shared background watch for the rest. Callers
    // (including main.cpp) don't need their own VendorUpdater instance.
...
private:
    ...
    // Non-owning — the real instance lives inside VendorManager, obtained
    // via registerVendor()+getUpdater() in start(). NOT a unique_ptr:
    // owning a second, separately-constructed VendorUpdater here was
    // exactly the bug that caused the binary path to silently diverge
    // from what VendorManager/the UI thinks is installed.
    vendor_updater::VendorUpdater* m_vendorUpdater = nullptr;
};
}